...
public class DigitalModulation
{
    ...
    public void QPSK()
    {
        ...
        var qsig = new Complex[x_symbols.len];
        
        //var phase_noise = Numpy.np.random.randn(x_symbols.len) * 0.1; //adjust multiplier for "strength" of phase noise
        //var dd = x_symbols * Numpy.np.exp(1j * phase_noise);
        
        for (var i = 0; i < x_symbols.len; i++)
        {
            qsig[i] = new Complex(real.GetData<double>()[i], img.GetData<double>()[i]);
        }
        ...
        var rsig = new Complex[num_symbols];
        ...
        {
            rsig[i] = qsig[i] + (new Complex(nsigq.GetData<double>()[i], nsigi.GetData<double>()[i]) * Math.Sqrt(noise_power) / Math.Sqrt(2));
        }

        var sig = Numpy.np.array(rsig);
